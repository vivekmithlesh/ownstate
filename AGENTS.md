...
# AGENTS.md — OwnState AI Dev Team (for Claude Code)

> **CLAUDE CODE: HOW TO USE THIS FILE**
>
> You will build OwnState like a real MNC engineering team. You play DIFFERENT
...
> - **No mock data in final code. Real Supabase data. Desktop + mobile. No compromise.**
...
- [ ] Uses REAL data (Supabase), no mock/hardcoded arrays in final code
...
- [ ] No secrets in code (only `.env.local`)
...
**Owns:** `supabase/schema.sql`, all SQL functions/RPCs, PostGIS columns, indexes, RLS policies, triggers, Prisma introspection.
**Standards:**
- Every table has RLS enabled with least-privilege policies.
- Geo columns are `geography(...,4326)`; GIST indexes on them.
...
**Owns:** `src/lib/actions/*` (server actions), `src/app/api/*` routes, `src/lib/supabase/*`, `src/lib/auth.ts`, payment server logic, `src/lib/queries.ts`.
**Standards:**
- Validate all inputs with zod before touching the DB.
- Server actions are `'use server'`; never leak the service-role key to the browser.
- Return typed results; handle and surface errors meaningfully.
- `revalidatePath` after mutations. Idempotent where it matters.
- Payment: verify Razorpay signatures server-side, never trust the client.

### 3. FRONTEND ENGINEER
**Mindset:** Builds clean, accessible, responsive UI that consumes real data.
**Owns:** all pages under `src/app/*` (except map/3D internals), `src/components/*` (cards, forms, navbar, footer, dashboard), wizard flows, forms.
**Standards:**
...
- Reuse `PropertyCard` and shared components — no duplicated UI.

### 4. MAPS / GEO ENGINEER
**Mindset:** Specialist in Leaflet + OpenStreetMap + PostGIS geo features.
**Owns:** `PropertyMap.tsx`, `FencingMap.tsx`, geo RPCs (with the Database Architect), Nominatim geocoding, polygon drawing (Leaflet-Geoman), area calc (@turf/turf).
**Standards:**
- Leaflet + OpenStreetMap tiles (free, no token). Esri World Imagery for satellite in fencing.
- Clean up map instances on unmount (no leaks). SSR-safe (dynamic import, `ssr:false`).
- Markers/polygons driven by real data; bounds queries hit PostGIS via RPC.
- Mobile: map full-width, touch-friendly controls, full-screen toggle on search.

### 5. 3D / CREATIVE ENGINEER
**Mindset:** Builds the cinematic React Three Fiber Earth hero. Performance-obsessed.
**Owns:** `src/components/earth/*`, scroll-progress hook, postprocessing, the homepage hero integration.
**Standards:**
- Deep-space cinematic look (per `BUILD_GUIDE.md` Brick 14), NOT minimalist.
- Procedural Earth fallback so it runs without texture files.
- Cap pixel ratio ≤2; points over heavy meshes; dispose geometries; pause useFrame off-screen.
- `prefers-reduced-motion` static fallback. Halve particle/star counts on mobile.
- Must hit ~60fps desktop, 30fps+ mid-range phone.
...
**Owns:** `.env` wiring, `next.config.js` (image domains), git hygiene, Vercel deploy, Supabase production URL config, CI sanity.
**Standards:**
- `.env.local` never committed. All env vars mirrored in Vercel.
- Image domains include Supabase storage + images.unsplash.com.
- Each brick ends with a git commit; v1.0 gets a tag at deploy.
- Production auth redirect URLs configured in Supabase.
...
| 01 | Database Schema | **Database Architect** | `supabase/schema.sql`, RLS, PostGIS, trigger |
| 02 | Prisma + Clients | Database Architect + Backend | Prisma introspection, Supabase clients, types |
| 03 | Seed Real Data | Backend + Database Architect | `scripts/seed.ts`, seed RPC, 24 real rows |
...
| 06 | Design System + Layout | **Frontend Engineer** | Tokens, Navbar, Footer, PropertyCard, layout |
| 07 | Home Page | Frontend (+ Backend for queries) | Real homepage with featured listings |
| 08 | Search + Map | **Maps/Geo** + Frontend | `PropertyMap`, search page, bounds geo-query |
| 09 | Property Detail | Frontend (+ Maps for mini-map) | Real detail page, EMI calc, enquiry |
| 10 | List Property | Frontend + Backend + Maps | 7-step wizard, real save, storage upload |
| 11 | Dashboard | Frontend + Backend | Real user dashboard + sub-pages |
| 12 | Land Fencing | **Maps/Geo** + Backend | `FencingMap`, polygon→PostGIS, fencing page |
| 13 | Deal Room + Payments | **Backend** + Frontend | Deal actions, Razorpay routes, deal room UI |
| 14 | 3D Earth Hero | **3D/Creative** + Frontend | `earth/*` components, scroll hero on homepage |
| 15 | Polish + Mobile + Deploy | QA + Frontend + **DevOps** | Responsive pass, SEO, error/404, Vercel deploy |
...
You are the OwnState engineering team. For every brick:
...
Rules: build brick by brick in order, never skip, no mock data (real Supabase),
desktop + mobile both, handle loading/empty/error states, no secrets in code,
do not compromise on the OwnState core features.
...
```
As the OwnState team (see AGENTS.md), begin BRICK <N>: <name>.
